...
def add(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.GET["categori"] == "water":
                if rpi.compare_time(request.POST["begin"], request.POST["end"], True):
                    schedule = WaterSchedule.objects.create(
                        begin=request.POST["begin"], end=request.POST["end"], rpi=rpi
                    )
                    schedule.save()
            ...
            context_dict = {"rpi": user_rpi, "form_phec": phec}
            ...
            return render(
                request, "schedule/add_schedule.html", {"form": form, "errors": errors}
            )
