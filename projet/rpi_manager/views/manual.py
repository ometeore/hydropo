...
def mode(request):
    ...
    water_schedule = list(WaterSchedule.objects.filter(rpi_id=1))

    light_schedule = list(LightSchedule.objects.filter(rpi_id=1))
    ...
    if request.GET["manual"] == "on":
        ...
        async_to_sync(channel_layer.group_send)(
            "group0", {"type": "send_message", "manual": True, "message": temp}
        )
