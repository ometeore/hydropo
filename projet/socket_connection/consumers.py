...
################ FUNCTIONS TO WORK WITH SYNC DB
    ...
    def database_conect_rpi(self):
        ...
        Rpi.objects.filter(uid_name = self.room_name).update(is_conected = True)
        Rpi.objects.filter(uid_name = self.room_name).update(last_connect = time_now)

    @database_sync_to_async
    def database_save_rpi_new_data(self, rpi_message):
        print(rpi_message)
        my_rpi = Rpi.objects.filter(uid_name = self.room_name)
        # new_ec = Ec.objects.create(
        #     date = datetime.now(),
        #     value = rpi_message['ec'],
        #     objectif = False,
        #     rpi = my_rpi
        # )
        # new_ec.save()
